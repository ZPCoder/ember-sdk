...
import { CARD_SET_DEFINITIONS, cardAvailableInRankedFormat } from "./formats.js";
import type { CardSetId } from "./types.js";
...
export type PackDrawOptions = {
  ...
  packType?: PackType;
  /** CSPRNG adapter supplied by Cocos, the browser, or the backend. */
  secureRng?: { fill(target: Uint32Array): void };
};
...
export function drawPack(
  ...
  const random = randomValues
    ...
    : (() => {
        ...
        if (options.secureRng) {
          options.secureRng.fill(values);
        } else {
          const cryptoApi = globalThis.crypto;
          if (!cryptoApi?.getRandomValues) {
            throw new Error("Opening packs requires a SecureRng adapter.");
          }
