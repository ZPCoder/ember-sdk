...
- 可选的三分钟新手训练：无需预先保存卡组，使用固定玩家牌组、固定对手、固定先手与种子重现同一教学牌序；战术教官与棱镜演算体会随阶段推进专属剧情台词；强制保留教学起手，并按“确认起手→使用晨辉斥候→结束回合→攻击敌方核心”逐动作解锁，非指定卡牌、目标和越级指令均被拦截；训练凭证不能结算正式战绩或奖励，失败后可保留已完成步骤并安全重试
